...
# %% isomap 
from src.tallem.dimred import isomap, connected_radius, enclosing_radius
# r = (connected_radius(N) + enclosing_radius(N))/2
# pc_iso = isomap(N, d=3, radius=r)

from sklearn.manifold import Isomap
embedding = Isomap(n_components=3)
pc_iso = embedding.fit_transform(N)

ax = plt.figure(figsize=(8,8)).add_subplot(projection='3d')
ax.scatter3D(*pc_iso.T, color=bin_color(knn_dist, col_pal))
...
P = np.array([[1,1], [2,1], [1.5, 2]]) # polytope
q = np.array([0, 0, 0]).reshape((3,1))
lb, ub = 0, 1


#%% Simplex projection (again)
import matplotlib.pyplot as plt

## Points in 2d 
polytope = np.array([[1,1], [2,1], [1.5, 2]]) 
d = polytope.shape[0]
I_d = np.eye(d)
P = np.vstack((np.c_[I_d, -I_d], np.c_[-I_d, I_d]))

q = np.array([0, 0, 0 ])
x = np.array([[5,5]]).T


p = pc.Polytope(C.equations[:,[0,1]], C.equations[:,[2]])




np.array([project_onto_standard_simplex(np.ravel(Y @ x))]) @ Y
